function Sr_tile () {
  ...
  /*  this.reDraw=function(){
    ...
    this.reDraw = function(a) {
        try {
            this.refresh();
            var incoming_data = window.data.sr;//this.globalSettings[a[0]];
            ...
            globals.renderComponent(globalSettings,{
                ...
                '                                            <div class="row">' +
                '                                                <div class="col-12">' +
                '                                                    <span class="tiles__wrapper__tile_title">Средний разряд</span><br>' +
                ...
                '                                                        </svg>' +
                '                                                    </button>' +
                '                                                    <strong class="tiles__wrapper__tile_values">' + incoming_data[0].fact + '</strong>' +
                '                                                </div>' +
                '                                            </div>' +
                '                                        </div>' +
